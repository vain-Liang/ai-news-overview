"""
Two-stage test for the Xinhua crawler schema.

Stage 1 (network, slow):  fetch live page via crawl4ai → save rendered HTML
Stage 2 (offline, fast):  apply xinhua CSS schema to saved HTML → verify articles

Run both in order:
    pytest tests/crawler/test_xinhua_extractor.py -v -m network   # fetch + save
    pytest tests/crawler/test_xinhua_extractor.py -v              # verify schema
"""
...
DATA_FILE = DATA_DIR / "xinhua.html"

_XINHUA_URL = "https://www.news.cn"


@pytest.mark.network
@pytest.mark.asyncio
async def test_fetch_xinhua_homepage_and_save() -> None:
    """Crawl www.news.cn and save the rendered HTML to tests/crawler/data/xinhua.html."""
    ...
    async with AsyncWebCrawler(config=browser_cfg) as crawler:
        result = await crawler.arun(url=_XINHUA_URL, config=run_cfg)
...
def test_xinhua_schema_extracts_articles() -> None:
    """Apply each xinhua CSS schema to saved HTML and assert valid article extraction."""
    if not DATA_FILE.exists():
        pytest.skip(f"Data file not found — run test_fetch_xinhua_homepage_and_save first: {DATA_FILE}")

    from crawl4ai.extraction_strategy import JsonCssExtractionStrategy

    from app.crawlers.parser import parse_articles
    from app.crawlers.xinhua import ALLOWED_DOMAINS, MAX_RESULTS, _SCHEMAS
    ...
    for schema in _SCHEMAS:
        ...
        raw = strategy.extract(url=_XINHUA_URL, html_content=html)
        items: list[dict] = json.loads(raw) if isinstance(raw, str) else (raw or [])

        articles = parse_articles(items, "xinhua", ALLOWED_DOMAINS, _XINHUA_URL, limit=MAX_RESULTS)
    ...
    pytest.fail(
        ...
        "then update baseSelector in src/app/crawlers/xinhua/__init__.py."
    )
