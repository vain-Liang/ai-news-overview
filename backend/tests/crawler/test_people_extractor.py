"""
Two-stage test for the People's Daily crawler schema.

Stage 1 (network, slow):  fetch live page via crawl4ai → save rendered HTML
Stage 2 (offline, fast):  apply People CSS schemas to saved HTML → verify articles

Run both in order:
    pytest tests/crawler/test_people_extractor.py -v -m network   # fetch + save
    pytest tests/crawler/test_people_extractor.py -v              # verify schema
"""
...
DATA_FILE = DATA_DIR / "people.html"

_PEOPLE_URL = "https://www.people.com.cn/"


@pytest.mark.network
@pytest.mark.asyncio
async def test_fetch_people_homepage_and_save() -> None:
    """Crawl www.people.com.cn and save the rendered HTML to tests/crawler/data/people.html."""
    ...
    run_cfg = CrawlerRunConfig(
        ...
        wait_for="css:ul#rmw_news li",
        page_timeout=60_000,
    )

    async with AsyncWebCrawler(config=browser_cfg) as crawler:
        result = await crawler.arun(url=_PEOPLE_URL, config=run_cfg)
...
def test_people_schema_extracts_articles() -> None:
    """Apply each People CSS schema to saved HTML and assert valid article extraction."""
    if not DATA_FILE.exists():
        pytest.skip(f"Data file not found — run test_fetch_people_homepage_and_save first: {DATA_FILE}")

    from crawl4ai.extraction_strategy import JsonCssExtractionStrategy

    from app.crawlers.parser import parse_articles
    from app.crawlers.people import ALLOWED_DOMAINS, MAX_RESULTS, _SCHEMAS
    ...
    for schema in _SCHEMAS:
        ...
        raw = strategy.extract(url=_PEOPLE_URL, html_content=html)
        items: list[dict] = json.loads(raw) if isinstance(raw, str) else (raw or [])

        articles = parse_articles(items, "peoples", ALLOWED_DOMAINS, _PEOPLE_URL, limit=MAX_RESULTS)
    ...
    pytest.fail(
        ...
        "then update baseSelector in src/app/crawlers/people/__init__.py."
    )
